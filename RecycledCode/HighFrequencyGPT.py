...
def apply_high_pass_filter(image, high_freq_removal_factor=1.9):
    ...
    # Remove a portion of high-frequency details based on high_freq_removal_factor
    high_freq_output *= (1 - high_freq_removal_factor)

    # Normalize the enhanced output image
    high_freq_output = (high_freq_output - np.min(high_freq_output)) / (
        np.max(high_freq_output) - np.min(high_freq_output))

    # Scale the output to 0-255 range
    high_freq_output = (high_freq_output * 255).astype(np.uint8)
...
def create_hybrid_image(high_freq_image_path, low_freq_image_path, high_freq_radius=80, blur_kernel_size=31):
    ...
    high_freq_output = apply_high_pass_filter(high_freq_image)
